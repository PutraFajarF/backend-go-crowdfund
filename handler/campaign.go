...
func (h *campaignHandler) UploadImage(c *gin.Context) {
	...
	// 2. ubah is_primary true ke false (untuk kasus kita melakukan upload dengan data is_primary = true maka data is_primary sebelumnya diubah ke false agar hanya ada satu yg true)

	var input campaign.CreateCampaignImageInput

	// Karena form (bukan json or uri) jadi pakai ShouldBind saja
	...
	if err != nil {
		...
		response := helper.APIResponse("Failed to upload campaign1 image", http.StatusBadRequest, "error", data)
	...
	if err != nil {
		data := gin.H{"is_uploaded": false}
		response := helper.APIResponse("Failed to upload campaign2 image", http.StatusBadRequest, "error", data)
	...
	if err != nil {
		data := gin.H{"is_uploaded": false}
		response := helper.APIResponse("Failed to upload campaign3 image", http.StatusBadRequest, "error", data)
